...
void round::notifyachiev (
    ...
    while (
      ...
      if (rounds_itr != _rounds.end()) {
        if (rounds_itr->round_status == name("end")) {
          ...
          switch(badge_round_status_iterator->balance_based_scoring_type) {
            ...
            case name("fibonacci"):
              balance_based_fibo_score (
                ...
                badge_round_status_iterator->balance_based_scoring_weight);
              break;
            default: check (false, "<name> scoring not supported");
          }
          switch(badge_round_status_iterator->source_based_scoring_type) {
            ...
            case name("fibonacci"):
              source_based_fibo_score (
                ...
                badge_round_status_iterator->source_based_scoring_weight);
              break;
            default: check(false, "<source based scoring> not supported");
          }
...
ACTION round::createround (name org,
  name round,
  string description,
...
ACTION round::activeround (name org, name round) {
  require_auth(org);
  rounds_table _rounds( _self, org.value );
  auto rounds_itr = _rounds.find(round.value);
...
ACTION round::endround (name org, name round) {
  require_auth(org);
  rounds_table _rounds( _self, org.value );
  auto rounds_itr = _rounds.find(round.value);
  check(rounds_itr != _rounds.end(), "<round> does not exist, create round using <createround> action");
...
ACTION round::pauseround (name org, name round) {
  require_auth(org);
  rounds_table _rounds( _self, org.value );
  auto rounds_itr = _rounds.find(round.value);
  check(rounds_itr != _rounds.end(), "<round> does not exist, create round using <createround> action");
...
ACTION round::resumeround (name org, name round) {
  require_auth(org);
  rounds_table _rounds( _self, org.value );
  auto rounds_itr = _rounds.find(round.value);
  check(rounds_itr != _rounds.end(), "<round> does not exist, create round using <createround> action");
...
ACTION round::addscoremeta (name org,
  name round, 
  uint64_t badge_id, 

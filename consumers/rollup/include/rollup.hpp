...
/* #undef SIMPLEBADGE_CONTRACT_NAME */
/* #undef CLAIMASSET_CONTRACT_NAME */
/* #undef NOTIFICATION_CONTRACT_NAME */
/* #undef ORCHESTRATOR_CONTRACT_NAME */
...
CONTRACT rollup : public contract {
  ...
  private:
    ...
    void invoke_action ( name org, std::map<asset_contract_name, uint16_t> emit_assets ,name to) {
    ...
    void emitters (name org, name emission_name, name account, name issuing_contract, name asset_name, uint8_t count) {
      ...
      if(insert_activated_row_in_accounts) {
        ...
        _accounts.emplace(get_self(), [&](auto& row){
          if(emit) {
            if(emissions_itr->cyclic_activator) {
              row.emission_status = ACTIVATOR_START;
            } else if (emissions_itr->cyclic_emitter) {
              row.emission_status = EMITTER_START;
            } else {

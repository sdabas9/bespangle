#include <bounties.hpp>
   ...
   ACTION bounties::cappedbounty( name             authorized,
                        name                       payer,
                        symbol                     badge_symbol,
                        bool                       new_badge,
      ...
      string action_name = "cappedbounty";
      ...
      if( !new_badge ) {
         check( badge_offchain_lookup_data.empty(), "offchain_lookup_data must be empty if new_badge is false" );
         check( badge_onchain_lookup_data.empty(),  "onchain_lookup_data must be empty if new_badge is false" );
      }
      ...
      if( closed_participants ) {
         check( !participants_list.empty(), "participants_list must have entries when closed_participants is true" );
      } else {
         check( participants_list.empty(), "participants_list must be empty when closed_participants is false" );
      }

      // 4. Call external action "nextemission" in the 'andemitter' contract.
      nextemission_args next_emission_data { org };
      action(
      ...
      // 5. Read the latest emission symbol from the autocode table.
      autocode_table ac_table( name(ANDEMITTER_CONTRACT), name(ANDEMITTER_CONTRACT).value );
      auto ac_itr = ac_table.find( org.value );
      check( ac_itr != ac_table.end(), "Autocode entry not found for organization" );
      symbol emission_symbol = ac_itr->last_auto_symbol;
      ...
      if( new_badge ) {
         ...
         action {
            ...
            createsimple_args {
               ...
               .offchain_lookup_data = badge_offchain_lookup_data,
               .onchain_lookup_data = badge_onchain_lookup_data,
               .memo = "" }
         }.send();
      ...
      new_emission(emission_symbol, 
         emission_offchain_lookup_data,
         emission_onchain_lookup_data,
         emitter_criteria, 
         emit_assets, 
         cyclic);
   ...
   ACTION bounties::addparts(name authorized, symbol badge_symbol, vector<name> participants) {
      // Ensure the authorized account signs the transaction.
      require_auth(authorized);

      // Open the bounty table.
      bounty_table bounty_tbl(get_self(), get_self().value);
      auto itr = bounty_tbl.find(badge_symbol.code().raw());
      check(itr != bounty_tbl.end(), "Bounty record not found for the given badge symbol");

      // Only allow adding participants if closed_participants is true.
      check(itr->closed_participants, "Bounty is not closed for participation; cannot add participants");
   ...
   ACTION bounties::addreviewers(name authorized, symbol badge_symbol, vector<name> reviewers) {
   ...
   void bounties::ontransfer(name from, name to, asset amount, string memo) {
      ...
      check(memo.size() > 0, "Memo must contain the bounty identifier (badge_symbol code string).");
   ...
   ACTION bounties::signup(name participant, symbol badge_symbol, string stream_reason) {
   ...
   ACTION bounties::cancelsignup(name participant, symbol badge_symbol, string stream_reason) {
   ...
   ACTION bounties::submit(name participant, symbol badge_symbol, string stream_reason) {
      require_auth(participant);
      bounty_table bounty_tbl(get_self(), get_self().value);
      auto itr = bounty_tbl.find(badge_symbol.code().raw());
      check(itr != bounty_tbl.end(), "Bounty record not found.");
      ...
      // Convert the bounty's badge_symbol.code() to a string, then to a name.
      name origin_key = name(itr->badge_symbol.code().to_string());
   ...
   ACTION bounties::withdraw(name account, symbol badge_symbol) {

      require_auth(account);
      // Lookup bounty record using badge_symbol as primary key.
      bounty_table bounty_tbl(get_self(), get_self().value);
      auto b_itr = bounty_tbl.find(badge_symbol.code().raw());
      check(b_itr != bounty_tbl.end(), "Bounty record not found for the given badge symbol.");
      ...
      if (account == b_itr->payer) {
         ...
         bounty_tbl.modify(b_itr, same_payer, [&](auto &row) {
    ...
    */
   ACTION bounties::closebounty(symbol badge_symbol) {
      bounty_table bounty_tbl(get_self(), get_self().value);
      auto itr = bounty_tbl.find(badge_symbol.code().raw());
      check(itr != bounty_tbl.end(), "Bounty record not found for the given badge symbol.");
   ...
   ACTION bounties::cleanup(symbol badge_symbol) {
      bounty_table bounty_tbl(get_self(), get_self().value);
      auto itr = bounty_tbl.find(badge_symbol.code().raw());
      check(itr != bounty_tbl.end(), "Bounty record not found for the given badge symbol.");
      ...
      deactivate_emission(itr->emission_symbol);

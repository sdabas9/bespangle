...
CONTRACT checks : public contract {
  ...
  private:
    ...
    name get_badge_for_antibadge (name org, name antibadge) {
      metadata_badge_table _badge(name(ORCHESTRATOR_CONTRACT_NAME), org.value);
      auto badge_index = _badge.get_index<name("badgename")>();
      auto badge_iterator = badge_index.find (antibadge.value);
      check(badge_iterator->badge_name == antibadge, "antibadge not created"); 
    ...
    vector<name> anti_badge_vector (name org, name badge) {
      metadata_badge_table _badge(name(ORCHESTRATOR_CONTRACT_NAME), org.value);
      auto badge_index = _badge.get_index<name("badgename")>();
      auto badge_iterator = badge_index.find (badge.value);
      check(badge_iterator->badge_name == badge, "UNEXPECTED: badge not created"); 
      auto onchain_lookup_data = json::parse(badge_iterator->onchain_lookup_data);
    ...
    uint64_t multiple_badges_additive_balance (name org, name account, vector<name> badges) {
      ...
      for (auto i = 0 ; i < badges.size(); i++) {
        uint64_t badge_id = get_badge_id(org, badges[i]);
        balance = balance + account_balance(org, account, badge_id);    
      }
      return balance;
    }

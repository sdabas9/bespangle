...
public:
  ...
  [[eosio::on_notify(SIMPLE_BADGE_CREATE_NOTIFICATION)]]
  ...
  ACTION executea(name org, name executing_account);
  ACTION executeb(name org, name executing_account);

  ACTION execute(name org, name executing_account, uint16_t step) {
    require_auth(executing_account);
    uint16_t processed = 0 ;
    if (step - processed > 0) {
      processed = processed + execute_a (org, step - processed);
    }    
    if (step - processed > 0) {
      processed = processed + execute_b (org, step - processed);
    }
    if (step - processed > 0) {
      processed = processed + execute_c (org, step - processed);
    }
    check(processed != 0, "Nothing to process");
  }

  ACTION executea(name org, name executing_account, uint16_t step) {
    require_auth(executing_account);
    uint16_t processed = 0 ;
    processed = execute_a(org, step);
    check(processed != step, "Nothing to process");
  }

  ACTION executeb(name org, name executing_account, uint16_t step) {
    require_auth(executing_account);
    uint16_t processed = 0 ;
    processed = execute_b(org, step);
    check(processed != step, "Nothing to process");
  }

  ACTION notifya(params) {
    require_recipient(notification_contract);
  }

  ACTION notifyb(params) {
    require_recipient(notification_contract);
  }

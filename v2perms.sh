...
# The accounts.txt file should define variables such as:
#   SIMPLE_MANAGER_CONTRACT, SIMPLEBADGE_CONTRACT, ORCHESTRATOR_CONTRACT,
...
#   1. The contract to update (sourced from accounts.txt)
#   2. The action name (literal)
#   3. The authorized contract (sourced from accounts.txt)
#
...
transactions=(
  ...
  "$ORCHESTRATOR_CONTRACT initbadge $SIMPLEBADGE_CONTRACT"
  "$ORCHESTRATOR_CONTRACT addfeature $SIMPLE_MANAGER_CONTRACT"
  "$SIMPLEBADGE_CONTRACT issue $SIMPLE_MANAGER_CONTRACT"
  "$ORCHESTRATOR_CONTRACT achievement $SIMPLEBADGE_CONTRACT"
  "$ANDEMITTER_CONTRACT newemission $ANDEMITTER_MANAGER_CONTRACT"
  "$ORCHESTRATOR_CONTRACT addfeature $ANDEMITTER_MANAGER_CONTRACT"
  "$ANDEMITTER_CONTRACT activate $ANDEMITTER_MANAGER_CONTRACT"
  "$SIMPLEBADGE_CONTRACT issue $ANDEMITTER_CONTRACT"
  "$ORCHESTRATOR_CONTRACT addfeature $BOUNDED_AGG_MANAGER_CONTRACT"
  ...
  "$ORCHESTRATOR_CONTRACT initbadge $MUTUAL_RECOGNITION_CONTRACT"
  "$ORCHESTRATOR_CONTRACT addfeature $MUTUAL_RECOGNITION_MANAGER_CONTRACT"
  "$MUTUAL_RECOGNITION_CONTRACT issue $MUTUAL_RECOGNITION_MANAGER_CONTRACT"
  "$ORCHESTRATOR_CONTRACT achievement $MUTUAL_RECOGNITION_CONTRACT"
  "$ORCHESTRATOR_CONTRACT addfeature $HLL_EMITTER_MANAGER_CONTRACT"
  "$HLL_EMITTER_VALIDATION_CONTRACT newemission $HLL_EMITTER_MANAGER_CONTRACT"
  "$HLL_EMITTER_VALIDATION_CONTRACT activate $HLL_EMITTER_MANAGER_CONTRACT"
  "$SIMPLEBADGE_CONTRACT issue $HLL_EMITTER_VALIDATION_CONTRACT"
  "$ORCHESTRATOR_CONTRACT addfeature $GIVER_REP_MANAGER_CONTRACT"
  ...
  "$ORCHESTRATOR_CONTRACT addfeature $BOUNDED_HLL_MANAGER_CONTRACT"
  ...
  "$SUBSCRIPTION_CONTRACT billing $HLL_EMITTER_VALIDATION_CONTRACT"
  ...
  "$ORCHESTRATOR_CONTRACT addfeature $BOUNTIES_CONTRACT"
...
for tx in "${transactions[@]}"; do
  # Use the default word-splitting on whitespace:
  set -- $tx
  push_auth "$1" "$2" "$3"
done
